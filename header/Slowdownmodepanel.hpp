...
inline void DrawLagSimulatorPanel(MidiOutputEngine& engine)
{
    ...
    if (ImGui::IsItemHovered()) {
        ImGui::SetTooltip("When ON: visualizer scrolls smoothly while audio drifts late.\nWhen OFF: visualizer physically stutters with audio.");
    }

    if (!enabled) {
        ImGui::TextDisabled("(disabled - playback runs at full native speed)");
        ImGui::Unindent(8.0f);
        return;
    }
    ...
    if (ImGui::InputScalar("##eps", ImGuiDataType_S64, &s_lagSimEps, &kStep, &kStepFast, "%lld")) {
        s_lagSimEps = std::clamp(s_lagSimEps, kLagSimMin, kLagSimMax);
    ...
    for (int i = 0; i < 14; ++i) {
        ...
        if (ImGui::IsItemHovered()) ImGui::SetTooltip("%s\n%lld EV/s", p.tip, (long long)p.eps);

        if (i < 13) {
            float lastButtonX2 = ImGui::GetItemRectMax().x;
            float nextButtonW  = ImGui::CalcTextSize(kPresets[i + 1].label).x + (style.FramePadding.x * 2.0f);
            float nextButtonX2 = lastButtonX2 + style.ItemSpacing.x + nextButtonW;
    ...
    if (lagging) {
        ...
        ImGui::TextWrapped("EV/s LIMITED (SLOWDOWN ACTIVE)");
        ImGui::PopStyleColor();
    } else {
